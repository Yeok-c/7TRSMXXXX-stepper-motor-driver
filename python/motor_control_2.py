...
class MotorController:
    ...
    def generate_serial_list(self, addr, func_code, reg_addr, num_regs, data_len, reg_value):
        # data before appending CRC
        ...
        assert len(data_to_crc)%2==0 # Just checking if problem is here

        # generate CRC
        p1, p2 = self.crc16_modbus(data_to_crc)

        # append CRC behind data
        data_with_crc = data_to_crc + self.int_hex2(p1) + self.int_hex2(p2)

        # append 0x to each item and form into string
    ...
    def n_step(self, n):
        ...
        data_len = 4
        reg_value = abs(n)
        mylist = self.generate_serial_list(addr, func_code, reg_addr, num_regs, data_len, reg_value)
    ...
    def move_to_thousand(self):
        mylist = [0x01, 0x10, 0x07, 0xD2, 0x00, 0x02, 0x04, 0x00, 0x00, 0x03, 0xe8, 0x59, 0xA4]
        self.ser_send(mylist)
